...
export default function AdminAmenitiesPage() {
  ...
  const fetchAmenities = async () => {
    try {
      ...
      setAmenities(Array.isArray(fetched) ? fetched : []);
    } catch (error) {
      console.error(error);
      setAmenities([]);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId ? `/api/admin/amenities/${editingId}` : "/api/admin/amenities";
  ...
  const filteredAmenities = amenities.filter(a => a.name?.includes(searchTerm));

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-navy-dark">المرافق</h1>
          <p className="text-gray-500 text-sm">إدارة المرافق والخدمات المتاحة للوحدات العقارية.</p>
        </div>
        {!showForm && (
          <button 
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm max-w-xl">
          <h3 className="font-bold text-navy-dark mb-4">{editingId ? "تعديل مرفق" : "إضافة مرفق جديد"}</h3>
          <div className="space-y-4">
            <div>
              ...
              <input 
                ...
                required
                placeholder="مثال: حديقة خاصة، جراج، أمن 24 ساعة"
            ...
            <table className="w-full text-right whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={3} className="px-6 py-8 text-center text-gray-400">لا توجد مرافق.</td>
                  </tr>
                ) : (
                  filteredAmenities.map(amenity => (
                    <tr key={amenity.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4 font-bold text-gray-400 text-sm">{amenity.id}</td>
                      <td className="px-6 py-4 font-bold text-navy-dark text-sm">{amenity.name}</td>
                      <td className="px-6 py-4 text-left">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => handleEdit(amenity)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors">
                            <FiEdit2 size={16} />
                          </button>
                          <button onClick={() => handleDelete(amenity.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                            <FiTrash2 size={16} />
                          </button>
                        </div>
                      </td>
